...
\begin{document}
    ...
    Our initial goal was to adapt the Lucid programming language to have first-class support for real-time programming\cite{bencina}, while retaining its nature as a general purpose language.
    ...
    This requires a heavily asynchronous mindset for which Lucid is a good starting point, since almost everything in Lucid is indeed asynchronous. It also requires avoiding the original Lucid implementation's reliance on indexed streams, since they worked based on a heuristic \cite[pages 67-68]{wadgeashcroft85} that would induce arbitrary recomputation in some cases. \footnote{The original implementation would store the last 200 or so steps of the computation, recomputing anything older than that on demand. The heuristic was that this was unnecessary most of the time. This is hardly useful if we want precise bounds on the amount of memory a Pixy program might use or how long it will take.}
...
x = if y % 2 == 0
    ...
    Often, delay compensation must be applied to the evaluation of a Pixy expression in order for it to operate correctly. \footnote{As an interesting analogy, this is quite similar to accounting for propagation delay in electronic circuit design. \cite{propwiki}}
...
c until c eq N where
    ...
    \begin{thebibliography}{9}
        ...
        \bibitem{bencina}
        Bencina, R.
        \textit{Real-time audio programming 101: time waits for nothing}
        \url{http://www.rossbencina.com/code/real-time-audio-programming-101-time-waits-for-nothing}, accessed 2018/04/25
        \bibitem{ltlwiki}
        Wikipedia
        \textit{Linear temporal logic}
        \url{https://en.wikipedia.org/wiki/Linear_temporal_logic}, accessed 2018/04/25    
        \bibitem{propwiki}
        Wikipedia
        Propagation delay
        \url{https://en.wikipedia.org/wiki/Propagation_delay}, accessed 2018/04/25
    \end{thebibliography}
